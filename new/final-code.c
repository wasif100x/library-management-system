...
// Display the book inventory and manage it
void manageInventory() {
    int choice;
...
void deleteBook() {
    ...
    fgets(deleteIsbn, sizeof(deleteIsbn), stdin);
    strtok(deleteIsbn, "\n");  // Remove newline character
    for (int i = 0; i < bookCount; i++) {
        if (strcmp(books[i].isbn, deleteIsbn) == 0) {
            ...
            saveBooksToFile();  // Save books to file after deletion
            return;
        }
    }
    printf("Book not found!\n");
...
void updateBook() {
    ...
    fgets(updateIsbn, sizeof(updateIsbn), stdin);
    strtok(updateIsbn, "\n");  // Remove newline character
    for (int i = 0; i < bookCount; i++) {
        if (strcmp(books[i].isbn, updateIsbn) == 0) {
            ...
            if (scanf("%d", &books[i].available) != 1) {
                printf("Invalid input. Availability should be 1 or 0.\n");
                clearInputBuffer();
                return;
            }
            clearInputBuffer();
            printf("Book updated successfully!\n");
            saveBooksToFile();  // Save books to file after updating
            return;
        }
    }
    printf("Book not found!\n");
